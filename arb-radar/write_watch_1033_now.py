#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-23 ~10:33 CST vs 09:35. Analysis only."""
...
# 429: reuse 09:35 TT vs LGD last-success quotes with †. JDG ended → keep —.
STALE_2233_LGD = {
    "stake_a": 2.25,
    "stake_b": 1.65,
    "duel_a": 2.28,
    ...
    "duel_updated": "2026-08-22T22:31:39.619Z",
}
lgd_books_missing = any(x is None for x in (stake_tt2, stake_lgd, duel_tt2, duel_lgd))
if dagger and lgd_books_missing:
    stake_tt2 = STALE_2233_LGD["stake_a"]
    stake_lgd = STALE_2233_LGD["stake_b"]
    duel_tt2 = STALE_2233_LGD["duel_a"]
    duel_lgd = STALE_2233_LGD["duel_b"]
...
if dagger:
    ...
    oai_live_lgd = False
    if not stake_upd:
        stake_upd = STALE_2233_LGD["stake_updated"]
    if not duel_upd:
        duel_upd = STALE_2233_LGD["duel_updated"]
...
# baseline 09:35 CST values
base_pm_tt, base_pm_lgd = 0.4, 0.61
base_stake_tt, base_stake_lgd = 2.25, 1.65
base_duel_tt, base_duel_lgd = 2.28, 1.68
base_pin_tt, base_pin_lgd = 2.31, 1.621
base_pin_listed = True
base_sz_tt, base_sz_lgd = 22864.18, 9940.66
base_rates = {
    "duel_tt": -0.05289796,  # Duel buy TT
    "stake_tt": -0.05811469,
    "duel_lgd": -3.809e-05,
...
base_best = -3.809e-05  # Duel buy LGD
...
if dagger:
    books_move_detail = "Stake/Duel 429 † reused 09:35, not treated as live move"
elif books_moved:
    books_move_detail = (
...
for book, r, thick in paths_sorted:
    if book == "PIN":
        ...
        note = "LPL Duel 429†沿用09:35，限额未知；本单按PM档1" if dagger else "LPL Duel现价，限额未知；本单按PM档1"
    else:
        note = "LPL Stake 429†沿用09:35，限额未知；本单按PM档1" if dagger else "LPL Stake现价，限额未知；本单按PM档1"
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，TT vs LGD 沿用 09:35 赛前价；JDG vs TT 已结束不沿用陈价。"
        if dagger
...
reason_txt = "；".join(reasons) if reasons else (
    ...
    "PM ask<0.02、Stake/Duel 无实质移动、PIN未新挂、未转滚球、档深变化不触发；上一轮已报 Duel 2.28/1.68、Stake 2.25/1.65、PIN 2.31/1.621 和 −0.00%。"
)
...
gaps = [
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 09:35 prematch quotes; JDG/TT not reused (ended)"
        if dagger
...
payload = {
    ...
    "sources": {
        ...
        "Stake": (
            ...
            + ("(429 †, reused 09:35); " if dagger else "(fresh, no 429); ")
    ...
    "vs_0935": {
        ...
        "TT vs LGD": {
            ...
            "stake": book_pair(base_stake_tt, base_stake_lgd, stake_tt2, stake_lgd) + ("（429†沿用09:35）" if dagger else ""),
            "duel": book_pair(base_duel_tt, base_duel_lgd, duel_tt2, duel_lgd) + ("（429†沿用09:35）" if dagger else ""),
            ...
            "path_pin_tt": (None if pin_tt is None else rate_pct_str(cur_rates.get("pin_tt", 0))),
            "path_pin_lgd": (None if pin_lgd is None else rate_pct_str(cur_rates.get("pin_lgd", 0))),
            "status": "非滚球→非滚球",
        },
