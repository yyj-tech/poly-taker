#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-22 ~18:34 CST vs 17:35. Analysis only."""
...
# 429: reuse 17:35 TT vs LGD last-success quotes with †. JDG ended → keep —.
STALE_1735_LGD = {
    "stake_a": 2.1,
    "stake_b": 1.75,
    "duel_a": 1.99,
    "duel_b": 1.88,
    "stake_updated": "2026-08-22T09:31:15.598Z",
...
if dagger and lgd_books_missing:
    stake_tt2 = STALE_1735_LGD["stake_a"]
    stake_lgd = STALE_1735_LGD["stake_b"]
    duel_tt2 = STALE_1735_LGD["duel_a"]
    duel_lgd = STALE_1735_LGD["duel_b"]
...
if dagger:
    ...
    oai_live_lgd = False
    if not stake_upd:
        stake_upd = STALE_1735_LGD["stake_updated"]
    if not duel_upd:
        duel_upd = STALE_1735_LGD["duel_updated"]
...
if dagger:
    books_move_detail = "Stake/Duel 429 † reused 17:35, not treated as live move"
elif books_moved:
    books_move_detail = (
...
for book, r, thick in paths_sorted:
    ...
    if dagger:
        note = ("LPL Duel 429†沿用17:35" if book == "Duel" else "LPL Stake 429†沿用17:35")
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，TT vs LGD 沿用 17:35 赛前价；JDG vs TT 已结束不沿用陈价。"
        if dagger
...
reason_txt = "；".join(reasons) if reasons else (
    ...
    "PM ask<0.02、Stake/Duel 无实质移动、未转滚球；上一轮已报 Stake 2.10/1.75 和 +0.24%。"
)
...
gaps = [
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 17:35 prematch quotes; JDG/TT not reused (ended)"
        if dagger
...
payload = {
    ...
    "sources": {
        ...
        "Stake": (
            ...
            + ("(429 †, reused 17:35); " if dagger else "(fresh, no 429); ")
    ...
    "vs_1735": {
        ...
        "lgd_l1_size_delta": round((s_b_lgd or 0) - 17633.22, 2),
        "tt_l1_size_delta": round((s_a_tt or 0) - 2074.75, 2),
    },
    "table1": [
        {
            "状态": "结束",
