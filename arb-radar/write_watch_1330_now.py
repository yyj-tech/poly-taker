#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-23 ~13:30 CST vs 12:31. Analysis only."""
...
# 429: reuse 12:31 TT vs LGD last-success quotes with †. JDG ended → keep —.
STALE_1231_LGD = {
    "stake_a": 2.35,
    "stake_b": 1.6,
    "duel_a": 2.38,
    "duel_b": 1.63,
    "stake_updated": "2026-08-23T04:29:25.166Z",
    "duel_updated": "2026-08-23T03:35:58.698Z",
}
lgd_books_missing = any(x is None for x in (stake_tt2, stake_lgd, duel_tt2, duel_lgd))
if dagger and lgd_books_missing:
    stake_tt2 = STALE_1231_LGD["stake_a"]
    stake_lgd = STALE_1231_LGD["stake_b"]
    duel_tt2 = STALE_1231_LGD["duel_a"]
    duel_lgd = STALE_1231_LGD["duel_b"]
...
if dagger:
    ...
    oai_live_lgd = False
    if not stake_upd:
        stake_upd = STALE_1231_LGD["stake_updated"]
    if not duel_upd:
        duel_upd = STALE_1231_LGD["duel_updated"]

# ---- paths TT vs LGD ----
settle = "约0.06天（8/23 15:00）"
paths = []
...
# baseline 12:31 CST values
base_pm_tt, base_pm_lgd = 0.4, 0.61
base_stake_tt, base_stake_lgd = 2.35, 1.6
base_duel_tt, base_duel_lgd = 2.38, 1.63
base_pin_tt, base_pin_lgd = 2.42, 1.571
base_pin_listed = True
base_sz_tt, base_sz_lgd = 20865.07, 10914.55
base_rates = {
    "duel_tt": -0.03607398,  # Duel buy TT
    "stake_tt": -0.04103219,
    "duel_lgd": -0.01796817,
    "stake_lgd": -0.02893766,
    "pin_tt": -0.02957759,
    "pin_lgd": -0.0396963,
}
base_best = -0.01796817  # Duel buy LGD
...
if dagger:
    books_move_detail = "Stake/Duel 429 † reused 12:31, not treated as live move"
elif books_moved:
    books_move_detail = (
...
for book, r, thick in paths_sorted:
    if book == "PIN":
        ...
        note = "LPL Duel 429†沿用12:31，限额未知；本单按PM档1" if dagger else "LPL Duel现价，限额未知；本单按PM档1"
    else:
        note = "LPL Stake 429†沿用12:31，限额未知；本单按PM档1" if dagger else "LPL Stake现价，限额未知；本单按PM档1"
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，TT vs LGD 沿用 12:31 赛前价；JDG vs TT 已结束不沿用陈价。"
        if dagger
...
md.append(
    ...
    + ("（429†沿用12:31）" if dagger else "")
    + "；"
    + f"Duel {book_pair(base_duel_tt, base_duel_lgd, duel_tt2, duel_lgd)}"
    + ("（429†沿用12:31）" if dagger else "")
    + "；"
    + f"PIN {book_pair(base_pin_tt, base_pin_lgd, pin_tt, pin_lgd)}。最佳利率 {rate_pct_str(base_best)}（Duel买LGD）→{rate_pct_str(best_rate)}（{best['book']}买{best['book_team']}，{pp_txt(best_pp)}个百分点）。"
...
reason_txt = "；".join(reasons) if reasons else (
    ...
    "PM ask<0.02、Stake/Duel 无实质移动、PIN未新挂、未转滚球、档深变化不触发；上一轮已报 Duel 2.38/1.63、Stake 2.35/1.6、PIN 2.42/1.571 和 −1.80%。"
)
...
gaps = [
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 12:31 prematch quotes; JDG/TT not reused (ended)"
        if dagger
...
payload = {
    ...
    "sources": {
        ...
        "Stake": (
            ...
            + ("(429 †, reused 12:31); " if dagger else "(fresh, no 429); ")
    ...
    "vs_1231": {
        ...
        "TT vs LGD": {
            ...
            "stake": book_pair(base_stake_tt, base_stake_lgd, stake_tt2, stake_lgd) + ("（429†沿用12:31）" if dagger else ""),
            "duel": book_pair(base_duel_tt, base_duel_lgd, duel_tt2, duel_lgd) + ("（429†沿用12:31）" if dagger else ""),
