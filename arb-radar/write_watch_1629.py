#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-22 ~16:32 CST vs 15:32. Analysis only."""
...
# 429: reuse 15:32 TT vs LGD last-success quotes with †. JDG ended → keep —.
STALE_1532_LGD = {
    "stake_a": 2.0,
    "stake_b": 1.82,
    "duel_a": 1.99,
...
if dagger and lgd_books_missing:
    stake_tt2 = STALE_1532_LGD["stake_a"]
    stake_lgd = STALE_1532_LGD["stake_b"]
    duel_tt2 = STALE_1532_LGD["duel_a"]
    duel_lgd = STALE_1532_LGD["duel_b"]
...
if dagger:
    ...
    oai_live_lgd = False
    if not stake_upd:
        stake_upd = STALE_1532_LGD["stake_updated"]
    if not duel_upd:
        duel_upd = STALE_1532_LGD["duel_updated"]
...
if dagger:
    books_move_detail = "Stake/Duel 429 † reused 15:32, not treated as live move"
elif books_moved:
    books_move_detail = (
...
for book, r, thick in paths_sorted:
    ...
    if dagger:
        note = ("LPL Duel 429†沿用15:32" if book == "Duel" else "LPL Stake 429†沿用15:32")
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，TT vs LGD 沿用 15:32 赛前价；JDG vs TT 已结束不沿用陈价。"
        if dagger
...
gaps = [
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 15:32 prematch quotes; JDG/TT not reused (ended)"
        if dagger
...
payload = {
    ...
    "sources": {
        ...
        "Stake": (
            ...
            + ("(429 †, reused 15:32); " if dagger else "(fresh, no 429); ")
    ...
    "vs_1532": {
        ...
        "lgd_l1_size_delta": round((s_b_lgd or 0) - 1167.87, 2),
        "tt_l1_size_delta": round((s_a_tt or 0) - 1571.94, 2),
    },
    "table1": [
        {
            "状态": "结束",
