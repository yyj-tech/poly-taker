#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-22 ~19:30 CST vs 18:35. Analysis only."""
...
# 429: reuse 18:35 TT vs LGD last-success quotes with †. JDG ended → keep —.
STALE_1835_LGD = {
    "stake_a": 2.15,
    "stake_b": 1.7,
    "duel_a": 1.99,
...
if dagger and lgd_books_missing:
    stake_tt2 = STALE_1835_LGD["stake_a"]
    stake_lgd = STALE_1835_LGD["stake_b"]
    duel_tt2 = STALE_1835_LGD["duel_a"]
    duel_lgd = STALE_1835_LGD["duel_b"]
...
if dagger:
    ...
    oai_live_lgd = False
    if not stake_upd:
        stake_upd = STALE_1835_LGD["stake_updated"]
    if not duel_upd:
        duel_upd = STALE_1835_LGD["duel_updated"]
...
if dagger:
    books_move_detail = "Stake/Duel 429 † reused 18:35, not treated as live move"
elif books_moved:
    books_move_detail = (
...
for book, r, thick in paths_sorted:
    ...
    if dagger:
        note = ("LPL Duel 429†沿用18:35" if book == "Duel" else "LPL Stake 429†沿用18:35")
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，TT vs LGD 沿用 18:35 赛前价；JDG vs TT 已结束不沿用陈价。"
        if dagger
...
gaps = [
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 18:35 prematch quotes; JDG/TT not reused (ended)"
        if dagger
...
payload = {
    ...
    "sources": {
        ...
        "Stake": (
            ...
            + ("(429 †, reused 18:35); " if dagger else "(fresh, no 429); ")
    ...
    "vs_1835": {
        ...
        "lgd_l1_size_delta": round((s_b_lgd or 0) - 17693.22, 2),
        "tt_l1_size_delta": round((s_a_tt or 0) - 3768.51, 2),
    },
    "table1": [
        {
            "状态": "结束",
