#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-22 ~12:28 CST vs 11:31. Analysis only."""
...
# 429: reuse 11:31 TT vs LGD last-success quotes with †. JDG ended → keep —.
STALE_1131_LGD = {
    "stake_a": 2.0,
    "stake_b": 1.82,
    "duel_a": 1.99,
...
if dagger and lgd_books_missing:
    stake_tt2 = STALE_1131_LGD["stake_a"]
    stake_lgd = STALE_1131_LGD["stake_b"]
    duel_tt2 = STALE_1131_LGD["duel_a"]
    duel_lgd = STALE_1131_LGD["duel_b"]
...
if dagger:
    ...
    oai_live_lgd = False
    if not stake_upd:
        stake_upd = STALE_1131_LGD["stake_updated"]
    if not duel_upd:
        duel_upd = STALE_1131_LGD["duel_updated"]
...
books_move_detail = (
    "Stake/Duel 429 † reused 11:31, not treated as live move"
    if dagger
...
for book, r, thick in paths_sorted:
    ...
    if dagger:
        note = ("LPL Duel 429†沿用11:31" if book == "Duel" else "LPL Stake 429†沿用11:31")
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，TT vs LGD 沿用 11:31 赛前价；JDG vs TT 已结束不沿用陈价。"
        if dagger
...
gaps = [
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 11:31 prematch quotes; JDG/TT not reused (ended)"
        if dagger
...
payload = {
    ...
    "sources": {
        ...
        "Stake": (
            ...
            + ("(429 †, reused 11:31); " if dagger else "(fresh, no 429); ")
    ...
    "vs_1131": {
        ...
        "tt_l1_size_delta": round((s_a_tt or 0) - 1464.72, 2),
    },
    "table1": [
        {
            "状态": "结束",
