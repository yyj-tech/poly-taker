#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-23 ~14:30 CST vs 13:35. Analysis only."""
...
STALE_1335_LGD = {
    ...
    "duel_a": 2.38,
    "duel_b": 1.63,
    "stake_updated": "2026-08-23T05:21:12.857Z",
...
base_pm_tt, base_pm_lgd = 0.39, 0.62
...
base_sz_tt, base_sz_lgd = 17105.44, 4877.73
base_rates = {
    "duel_tt": -0.04532024,  # Duel buy TT
    "stake_tt": -0.05018402,
    "duel_lgd": -0.00807032,
    "stake_lgd": -0.01926079,
    "pin_tt": -0.03894829,
    "pin_lgd": -0.03023371,
}
base_best = -0.00807032  # Duel buy LGD
...
for book, r, thick in paths_sorted:
    if book == "PIN":
        ...
        note = "LPL Duel 429†沿用13:35，限额未知；本单按PM档1" if dagger else "LPL Duel现价，限额未知；本单按PM档1"
    else:
        note = "LPL Stake 429†沿用13:35，限额未知；本单按PM档1" if dagger else "LPL Stake现价，限额未知；本单按PM档1"
...
reason_txt = "；".join(reasons) if reasons else (
    ...
    "PM ask<0.02、Stake/Duel 无实质移动、PIN未新挂、未转滚球、档深变化不触发；上一轮已报 Duel 2.38/1.63、Stake 2.35/1.6、PIN 2.42/1.571 和 −0.81%。"
)
...
payload = {
    ...
    "vs_1335": {
        ...
        "TT vs LGD": {
            ...
            "stake": book_pair(base_stake_tt, base_stake_lgd, stake_tt2, stake_lgd) + ("（429†沿用13:35）" if dagger else ""),
            "duel": book_pair(base_duel_tt, base_duel_lgd, duel_tt2, duel_lgd) + ("（429†沿用13:35）" if dagger else ""),
