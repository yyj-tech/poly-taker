#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-22 ~21:37 CST vs 20:33. Analysis only."""
...
# 429: reuse 20:33 TT vs LGD last-success quotes with †. JDG ended → keep —.
STALE_2033_LGD = {
    "stake_a": 2.15,
    "stake_b": 1.7,
    "duel_a": 2.1,
    ...
    "duel_updated": "2026-08-22T11:40:28.66Z",
}
lgd_books_missing = any(x is None for x in (stake_tt2, stake_lgd, duel_tt2, duel_lgd))
if dagger and lgd_books_missing:
    stake_tt2 = STALE_2033_LGD["stake_a"]
    stake_lgd = STALE_2033_LGD["stake_b"]
    duel_tt2 = STALE_2033_LGD["duel_a"]
    duel_lgd = STALE_2033_LGD["duel_b"]
...
if dagger:
    ...
    oai_live_lgd = False
    if not stake_upd:
        stake_upd = STALE_2033_LGD["stake_updated"]
    if not duel_upd:
        duel_upd = STALE_2033_LGD["duel_updated"]
...
# baseline 20:33 CST values
base_pm_tt, base_pm_lgd = 0.46, 0.55
base_stake_tt, base_stake_lgd = 2.15, 1.7
base_duel_tt, base_duel_lgd = 2.1, 1.79
base_sz_tt, base_sz_lgd = 4955.46, 17605.61
base_rates = {
    "duel_tt": -0.03194272,  # Duel buy TT
    "stake_tt": -0.02145224,
    "duel_lgd": -0.02379254,
    "stake_lgd": -0.05118703,
}
base_best = -0.02145224
...
if dagger:
    books_move_detail = "Stake/Duel 429 † reused 20:33, not treated as live move"
elif books_moved:
    books_move_detail = (
...
for book, r, thick in paths_sorted:
    ...
    if dagger:
        note = ("LPL Duel 429†沿用20:33" if book == "Duel" else "LPL Stake 429†沿用20:33")
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，TT vs LGD 沿用 20:33 赛前价；JDG vs TT 已结束不沿用陈价。"
        if dagger
...
md.append(
    ...
    + ("（429†陈价）" if dagger else "")
    + "；"
    + f"PIN 仍无。最佳利率 {rate_pct_str(base_best)}（Stake买TT）→{rate_pct_str(best_rate)}（{best['book']}买{best['book_team']}，{pp_txt(best_pp)}个百分点）。"
...
reason_txt = "；".join(reasons) if reasons else (
    ...
    "PM ask<0.02、Stake/Duel 无实质移动、未转滚球、档深变化不触发；上一轮已报 Stake 2.15/1.70 和 +0.24%。"
)
...
gaps = [
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 20:33 prematch quotes; JDG/TT not reused (ended)"
        if dagger
...
payload = {
    ...
    "sources": {
        ...
        "Stake": (
            ...
            + ("(429 †, reused 20:33); " if dagger else "(fresh, no 429); ")
    ...
    "vs_2033": {
        ...
        "lgd_l1_size_delta": round((s_b_lgd or 0) - 17605.61, 2),
        "tt_l1_size_delta": round((s_a_tt or 0) - 4955.46, 2),
    },
    "table1": [
        {
            "状态": "结束",
