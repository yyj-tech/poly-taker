#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-21 ~15:30 CST vs 14:34. Analysis only."""
...
l1_thick = bool(s_b_lgd and s_b_lgd > 2000)
...
paths = []
# Book buy TT / PM buy LGD (uses thick LGD ask)
...
# baseline 14:34 values
base_pm_tt, base_pm_lgd = 0.51, 0.5
base_stake_tt, base_stake_lgd = 1.92, 1.88
base_duel_tt, base_duel_lgd = 1.93, 1.93
base_sz_tt, base_sz_lgd = 317.7, 2972.6
base_rates = {
    "duel_tt": -0.02380426,  # Duel buy TT
    "stake_tt": -0.02636917,
    "duel_lgd": -0.03335614,
    "stake_lgd": -0.04606308,
}
base_best = -0.02380426
...
for book, r, thick in paths_sorted:
    ...
    thick_note = "；档1异常厚" if thick and l1_thick else ""
...
md.append(
    ...
    f"Stake/Duel 经 odds-api.io LPL Split 3 本轮新刷成功（{'有429†' if dagger else '无429，无†'}）；"
...
live_line = (
    f"是否滚球：否。JDG vs TT 已结束（昨日 17:00 CST 开赛，赛后约 {mins_past:.0f} 分钟；"
    ...
    f"odds-api.io 本轮新刷，JDG/TT 不在 pending/live；PIN 父盘 1633881950 absent_from_list，"
...
md.append(
    f"Stake/Duel 本轮 odds-api.io 新刷成功，{'有 429†' if dagger else '无 429，无†'}。"
    f"JDG vs TT 事件 7291391184 已不在 pending/live 且 odds_multi 未返回，表1 Stake/Duel 写—（不沿用陈价）。"
    f"TT vs LGD id=5958057944 status={oai_status_lgd} live={str(oai_live_lgd).lower()} "
    f"Stake {stake_tt2}/{stake_lgd} Duel {duel_tt2}/{duel_lgd}。"
...
if gt_0p5_paths:
    ...
    md.append(
        ...
        f"TT vs LGD 最佳 Duel 买TT {best_txt}，不升格。"
    )
md.append("")
md.append(
...
# TT vs LGD changes
sz_tt_chg = f"{base_sz_tt:g}@0.51→{sz_tt_s}份 @{pm_a_tt}"
sz_lgd_chg = f"{base_sz_lgd:g}@0.5→{sz_lgd_s}份 @{pm_b_lgd}"
md.append(
    ...
    f"PIN 仍无。最佳利率 {rate_pct_str(base_best)}（Duel买TT）→{rate_pct_str(best_rate)}（Duel买TT，{pp_txt(best_pp)}个百分点）。"
...
gaps = [
    ...
    f"odds-api.io fresh this round ({'429†' if dagger else 'no 429'}); "
    f"JDG/TT 7291391184 absent from pending/live; TT/LGD 5958057944 {oai_status_lgd} "
    f"Stake {stake_tt2}/{stake_lgd} Duel {duel_tt2}/{duel_lgd}",
    ...
    f"TT vs LGD TT L1 {base_sz_tt}@0.51→{sz_tt_s}@{pm_a_tt}; "
    f"LGD L1 {base_sz_lgd}@0.5→{sz_lgd_s}@{pm_b_lgd}",
...
payload = {
    ...
    "sources": {
        ...
        "Stake": (
            f"odds-api.io Split 3 {SCAN} CST (fresh, {'429' if dagger else 'no 429'}); "
    ...
    "vs_1434": {
        ...
        "TT vs LGD": {
            "pm": f"{base_pm_tt:g}/{base_pm_lgd:g}→{pm_a_tt:g}/{pm_b_lgd:g}"
            if pm_a_tt == base_pm_tt and pm_b_lgd == base_pm_lgd
            else f"{base_pm_tt:g}/{base_pm_lgd:g}→{pm_a_tt}/{pm_b_lgd}",
            ...
            "sz_tt": f"{base_sz_tt}@0.51→{sz_tt_s}@{pm_a_tt}",
            "sz_lgd": f"{base_sz_lgd}@0.5→{sz_lgd_s}@{pm_b_lgd}",
        ...
        "books_move_detail": "no live book move" if not books_moved else "books moved",
        "flip": flip,
        "oai_429": dagger,
        ...
        "lgd_l1_size_delta": round((s_b_lgd or 0) - 2972.58, 2),
    },
    "table1": [
        {
            "状态": "结束",
...
# fix pm pair display when unchanged
if pm_a_tt == base_pm_tt and pm_b_lgd == base_pm_lgd:
    payload["vs_1434"]["TT vs LGD"]["pm"] = f"{base_pm_tt:g}/{base_pm_lgd:g}→{pm_a_tt:g}/{pm_b_lgd:g}"
