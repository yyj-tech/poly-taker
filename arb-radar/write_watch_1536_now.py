#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-23 ~15:36 CST vs 14:33. Analysis only."""
...
# 429 reuse 14:33 quotes with †
STALE_1433_LGD = {
    "stake_a": 2.4,
    "stake_b": 1.58,
    "duel_a": 2.42,
    "duel_b": 1.6,
    "stake_updated": "2026-08-23T06:18:41.656Z",
    "duel_updated": "2026-08-23T06:11:47.79Z",
}
lgd_books_missing = any(x is None for x in (stake_tt2, stake_lgd, duel_tt2, duel_lgd))
if dagger and lgd_books_missing:
    stake_tt2 = STALE_1433_LGD["stake_a"]
    stake_lgd = STALE_1433_LGD["stake_b"]
    duel_tt2 = STALE_1433_LGD["duel_a"]
    duel_lgd = STALE_1433_LGD["duel_b"]
...
if dagger:
    ...
    if not stake_upd:
        stake_upd = STALE_1433_LGD["stake_updated"]
    if not duel_upd:
        duel_upd = STALE_1433_LGD["duel_updated"]
...
# baseline 14:33 CST
base_pm_tt, base_pm_lgd = 0.38, 0.63
base_stake_tt, base_stake_lgd = 2.4, 1.58
base_duel_tt, base_duel_lgd = 2.42, 1.6
base_pin_tt, base_pin_lgd = 2.45, 1.556
base_pin_listed = True
base_sz_tt, base_sz_lgd = 24274.23, 17938.34
base_rates = {
    "duel_tt": -0.04813196,
    "stake_tt": -0.05124178,
    "duel_lgd": -0.00938744,
    "stake_lgd": -0.01709062,
    "pin_tt": -0.04352526,
    "pin_lgd": -0.02643229,
}
base_best = -0.00938744  # Duel buy LGD
...
if dagger:
    books_move_detail = "Stake/Duel 429 † reused 14:33, not treated as live move"
elif books_moved:
    books_move_detail = (
...
for book, r, thick in paths_sorted:
    ...
    if book == "PIN":
        ...
        note = "LPL Duel 429†沿用14:33，限额未知；本单按PM档1" if dagger else "LPL Duel现价，限额未知；本单按PM档1"
    else:
        note = "LPL Stake 429†沿用14:33，限额未知；本单按PM档1" if dagger else "LPL Stake现价，限额未知；本单按PM档1"
    extra = []
    extra.append("不升格")
    if is_live:
        extra.append("滚球规则风险")
...
if pin_period0_withdrawn_tt:
    reasons.append("PIN period-0 已撤")
if is_live:
...
md.append("")
live_note = "TT vs LGD 已转滚球（SEND）。" if is_live else "TT vs LGD 仍非滚球。"
end_note = (
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，沿用 14:33 价。"
        if dagger
...
md.append(
    ...
    f"TT vs LGD **已转滚球**：PM live={gamma_live_lgd}、closed={gamma_closed_lgd}、ended={gamma_ended_lgd}、"
...
md.append(
    ...
    + (
        ...
        if tt_ended
        else f"TT vs LGD **未结束、已开赛滚球** score {gamma_score_lgd}（0-0 Bo3，地图1进行中）。"
    )
...
md.append(
    ...
    + ("（429†沿用14:33）" if dagger else "")
    + "；"
    + f"Duel {book_pair(base_duel_tt, base_duel_lgd, duel_tt2, duel_lgd)}"
    + ("（429†沿用14:33）" if dagger else "")
    + "；"
    + f"PIN {base_pin_tt:g}/{base_pin_lgd:g}→{pin_now}（period-0 系列 ML 已撤）。"
    + f"最佳利率 {rate_pct_str(base_best)}（Duel买LGD）→{rate_pct_str(best_rate)}"
    ...
    + f"PIN 买LGD {rate_pct_str(base_rates['pin_lgd'])}→—。"
    ...
    + f"PIN 买TT {rate_pct_str(base_rates['pin_tt'])}→—。"
    + f"状态 非滚球→{status_lgd}。"
)
...
md.append(
    ...
    f"TT vs LGD **已转滚球、未结束**（PM live={gamma_live_lgd} closed={gamma_closed_lgd} ended={gamma_ended_lgd} "
...
gaps = [
    ...
    + (
        f"TT vs LGD 父盘 {pin_tt_parent_id} 在列表但 period-0 系列 ML 不在 /markets/straight（parent_mk={parent_tt_mk} 仅 period 2/3）；"
        f"地图子盘 1634593525 isLive=True period-0 ML closed，不当系列。"
        if pin_period0_withdrawn_tt
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 14:33 quotes; JDG/TT not reused (ended)"
        if dagger
    ...
    (
        "routine not deleted: JDG vs TT ended (score 000-000|2-1|Bo3), TT vs LGD LIVE 000-000|0-0|Bo3. "
...
payload = {
    ...
    "sources": {
        ...
        "Stake": (
            ...
            + ("(429 †, reused 14:33); " if dagger else "(fresh, no 429); ")
    ...
    "vs_1433": {
        ...
        "TT vs LGD": {
            ...
            + ("（429†沿用14:33）" if dagger else ""),
            "pin": f"{base_pin_tt:g}/{base_pin_lgd:g}→— (period-0 withdrawn)",
            ...
            "path_pin_tt": f"{rate_pct_str(base_rates['pin_tt'])}→—",
            "path_pin_lgd": f"{rate_pct_str(base_rates['pin_lgd'])}→—",
            "status": f"非滚球→{status_lgd}",
            "went_live": True,
            "pin_period0": "open→withdrawn",
        },
        "ge_1p5": False,
