#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-21 ~22:28 CST vs 21:29. Analysis only."""
...
# 429: reuse 16:34 TT vs LGD last-success quotes with † (17:35 also reused). JDG ended → keep —.
STALE_1634_LGD = {
    "stake_a": 1.92,
    "stake_b": 1.88,
    "duel_a": 1.93,
    "duel_b": 1.93,
    "stake_updated": "2026-08-20T13:28:57.13Z",
    "duel_updated": "2026-08-20T08:50:05.088Z",
}
lgd_books_missing = any(x is None for x in (stake_tt2, stake_lgd, duel_tt2, duel_lgd))
if dagger and lgd_books_missing:
    stake_tt2 = STALE_1634_LGD["stake_a"]
    stake_lgd = STALE_1634_LGD["stake_b"]
    duel_tt2 = STALE_1634_LGD["duel_a"]
    duel_lgd = STALE_1634_LGD["duel_b"]
...
if dagger:
    ...
    oai_live_lgd = False
    if not stake_upd:
        stake_upd = STALE_1634_LGD["stake_updated"]
    if not duel_upd:
        duel_upd = STALE_1634_LGD["duel_updated"]
...
# baseline 20:32 values
base_pm_tt, base_pm_lgd = 0.48, 0.53
base_stake_tt, base_stake_lgd = 1.92, 1.88
base_duel_tt, base_duel_lgd = 1.93, 1.93
base_sz_tt, base_sz_lgd = 110.45, 927.44
base_rates = {
    "duel_tt": -0.05189533,  # Duel buy TT
    "stake_tt": -0.05431493,
    "duel_lgd": -0.00410817,
    "stake_lgd": -0.01759033,
}
base_best = -0.00410817
...
books_move_detail = (
    "Stake/Duel 429 † reused 16:34, not treated as live move"
    if dagger
    else ("no live book move" if not books_moved else "books moved")
)
...
for book, r, thick in paths_sorted:
    ...
    if dagger:
        note = ("LPL Duel 429†沿用16:34" if book == "Duel" else "LPL Stake 429†沿用16:34")
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，TT vs LGD 沿用 16:34 赛前价；JDG vs TT 已结束不沿用陈价。"
        if dagger
...
# data gaps
gap_parents = "；".join(parent_desc) if parent_desc else "listed_parents=none（TES/BLG 1633979763 上一轮在列、本轮已撤）"
gaps = [
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 16:34 prematch quotes; JDG/TT not reused (ended)"
        if dagger
...
payload = {
    ...
    "sources": {
        ...
        "PIN": (
            ...
            + (
                ...
                if parents
                else "none (TES/BLG 1633979763 gone vs 21:29)"
            )
        ),
        "Stake": (
            f"odds-api.io Split 3 {SCAN} CST "
            + ("(429 †, reused 16:34); " if dagger else "(fresh, no 429); ")
    ...
    "vs_2129": {
        ...
        "lgd_l1_size_delta": round((s_b_lgd or 0) - 927.44, 2),
        "tt_l1_size_delta": round((s_a_tt or 0) - 110.45, 2),
    },
    "table1": [
        {
            "状态": "结束",
