#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-22 ~10:28 CST vs 09:34. Analysis only."""
...
# 429: reuse 09:34 TT vs LGD last-success quotes with †. JDG ended → keep —.
STALE_0934_LGD = {
    "stake_a": 1.98,
    "stake_b": 1.82,
    "duel_a": 1.99,
    "duel_b": 1.88,
    "stake_updated": "2026-08-22T01:01:26.932Z",
...
if dagger and lgd_books_missing:
    stake_tt2 = STALE_0934_LGD["stake_a"]
    stake_lgd = STALE_0934_LGD["stake_b"]
    duel_tt2 = STALE_0934_LGD["duel_a"]
    duel_lgd = STALE_0934_LGD["duel_b"]
...
if dagger:
    ...
    oai_live_lgd = False
    if not stake_upd:
        stake_upd = STALE_0934_LGD["stake_updated"]
    if not duel_upd:
        duel_upd = STALE_0934_LGD["duel_updated"]
...
# baseline 09:34 CST values
base_pm_tt, base_pm_lgd = 0.46, 0.55
base_stake_tt, base_stake_lgd = 1.98, 1.82
base_duel_tt, base_duel_lgd = 1.99, 1.88
base_sz_tt, base_sz_lgd = 1520.98, 1609.15
base_rates = {
...
books_move_detail = (
    "Stake/Duel 429 † reused 09:34, not treated as live move"
    if dagger
...
for book, r, thick in paths_sorted:
    ...
    if dagger:
        note = ("LPL Duel 429†沿用09:34" if book == "Duel" else "LPL Stake 429†沿用09:34")
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，TT vs LGD 沿用 09:34 赛前价；JDG vs TT 已结束不沿用陈价。"
        if dagger
...
gaps = [
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 09:34 prematch quotes; JDG/TT not reused (ended)"
        if dagger
...
payload = {
    ...
    "sources": {
        ...
        "Stake": (
            ...
            + ("(429 †, reused 09:34); " if dagger else "(fresh, no 429); ")
    ...
    "vs_0934": {
        ...
        "lgd_l1_size_delta": round((s_b_lgd or 0) - 1609.15, 2),
        "tt_l1_size_delta": round((s_a_tt or 0) - 1520.98, 2),
    },
    "table1": [
        {
            "状态": "结束",
