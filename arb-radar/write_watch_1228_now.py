#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-23 ~12:28 CST vs 11:34. Analysis only."""
...
# match_watch may miss ThunderTalk/TT alias; always prefer pulled id 5958057944
...
# 429: reuse 11:34 TT vs LGD last-success quotes with †. JDG ended → keep —.
STALE_1134_LGD = {
    "stake_a": 2.25,
    "stake_b": 1.65,
    "duel_a": 2.36,
    "duel_b": 1.64,
    "stake_updated": "2026-08-22T18:33:19.856Z",
    "duel_updated": "2026-08-23T03:21:50.361Z",
}
lgd_books_missing = any(x is None for x in (stake_tt2, stake_lgd, duel_tt2, duel_lgd))
if dagger and lgd_books_missing:
    stake_tt2 = STALE_1134_LGD["stake_a"]
    stake_lgd = STALE_1134_LGD["stake_b"]
    duel_tt2 = STALE_1134_LGD["duel_a"]
    duel_lgd = STALE_1134_LGD["duel_b"]

mu = json.loads((SCANS / "pin" / "LPL_matchups.json").read_text())
rel_path = SCANS / "pin" / "LPL_markets_related.json"
if rel_path.exists():
    mk_rel_raw = json.loads(rel_path.read_text())
else:
    mk_rel_raw = {"status": 404}
...
# TT/LGD parent search + period-0 ML
...
if dagger:
    ...
    oai_live_lgd = False
    if not stake_upd:
        stake_upd = STALE_1134_LGD["stake_updated"]
    if not duel_upd:
        duel_upd = STALE_1134_LGD["duel_updated"]

# ---- paths TT vs LGD ----
settle = "约0.1天（8/23 15:00）"
paths = []
...
# PIN vs PM
if s_b_lgd and pm_b_lgd is not None and pin_tt:
    paths.append(
        (
            "PIN",
            path(s_b_lgd, pm_b_lgd, pin_tt, "PIN", "TT", "LGD", pin_limit=pin_tt_limit),
            bool(s_b_lgd and s_b_lgd > 2000),
        )
    )
if s_a_tt and pm_a_tt is not None and pin_lgd:
    paths.append(
        (
            "PIN",
            path(s_a_tt, pm_a_tt, pin_lgd, "PIN", "LGD", "TT", pin_limit=pin_tt_limit),
            bool(s_a_tt and s_a_tt > 2000),
        )
    )

# sort by profit desc (rate order coincides this round)
paths_sorted = sorted(paths, key=lambda x: x[1]["profit"], reverse=True)
best_profit = paths_sorted[0][1] if paths_sorted else None
best = max(paths, key=lambda x: x[1]["rate"])[1] if paths else None

# baseline 11:34 CST values
base_pm_tt, base_pm_lgd = 0.4, 0.61
base_stake_tt, base_stake_lgd = 2.25, 1.65
base_duel_tt, base_duel_lgd = 2.36, 1.64
base_pin_tt, base_pin_lgd = 2.48, 1.543
base_pin_listed = True
base_sz_tt, base_sz_lgd = 34661.58, 22174.66
base_rates = {
    "duel_tt": -0.03937115,  # Duel buy TT
    "stake_tt": -0.05811469,
    "duel_lgd": -0.01434726,
    "stake_lgd": -0.01074392,
    "pin_tt": -0.02007067,
    "pin_lgd": -0.05023149,
}
base_best = -0.01074392  # Stake buy LGD
...
if dagger:
    books_move_detail = "Stake/Duel 429 † reused 11:34, not treated as live move"
elif books_moved:
    books_move_detail = (
...
for book, r, thick in paths_sorted:
    if book == "PIN":
        ...
        note = "LPL Duel 429†沿用11:34，限额未知；本单按PM档1" if dagger else "LPL Duel现价，限额未知；本单按PM档1"
    else:
        note = "LPL Stake 429†沿用11:34，限额未知；本单按PM档1" if dagger else "LPL Stake现价，限额未知；本单按PM档1"
    thick_note = "；档1异常厚" if thick else ""
    hv_note = "不升格" if not r["hv"] else "可升格"
    event = f"TT vs LGD（{note}；{hv_note}{thick_note}）"
...
pin_newly = (not base_pin_listed) and pin_tt_lgd_listed and pin_tt is not None and pin_lgd is not None
pin_moved = False
if base_pin_listed and pin_tt is not None and pin_lgd is not None:
    pin_moved = abs(pin_tt - base_pin_tt) >= 0.02 or abs(pin_lgd - base_pin_lgd) >= 0.02
if pin_newly:
    reasons.append("PIN新挂盘")
if pin_moved:
    reasons.append("PIN价格实质移动")
...
for c in live_children:
    ...
    live_desc.append(f"LIVE_CHILD {names} {c['id']} parent={c['parentId']} status={c['status']} units={c.get('units')}")
if live_desc:
    parent_desc.extend(live_desc)
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，TT vs LGD 沿用 11:34 赛前价；JDG vs TT 已结束不沿用陈价。"
        if dagger
...
md.append(
    f"| 非滚球 | TT vs LGD | TT | LGD | {pm_cell(pm_a_tt)} | {pm_cell(pm_b_lgd)} | {pm_cell(pin_tt)} | {pm_cell(pin_lgd)} | "
...
md.append(
    ...
    + (
        ...
        else "PIN 本轮仍无 TT vs LGD 父对阵（8/23 未挂上）。"
    )
)
md.append("")
md.append("## 表2 套现仓位")
...
md.append(
    ...
    + ("（429†沿用11:34）" if dagger else "")
    + "；"
    + f"Duel {book_pair(base_duel_tt, base_duel_lgd, duel_tt2, duel_lgd)}"
    + ("（429†沿用11:34）" if dagger else "")
    + "；"
    + f"PIN {book_pair(base_pin_tt, base_pin_lgd, pin_tt, pin_lgd)}。最佳利率 {rate_pct_str(base_best)}（Stake买LGD）→{rate_pct_str(best_rate)}（{best['book']}买{best['book_team']}，{pp_txt(best_pp)}个百分点）。"
    ...
    + f"PIN 买LGD {rate_pct_str(base_rates['pin_lgd'])}→{rate_pct_str(cur_rates.get('pin_lgd', base_rates['pin_lgd']))}。"
    ...
    + f"PIN 买TT {rate_pct_str(base_rates['pin_tt'])}→{rate_pct_str(cur_rates.get('pin_tt', base_rates['pin_tt']))}。"
    + "状态仍非滚球。"
)
...
reason_txt = "；".join(reasons) if reasons else (
    ...
    "PM ask<0.02、Stake/Duel 无实质移动、PIN未新挂、未转滚球、档深变化不触发；上一轮已报 Duel 2.36/1.64、Stake 2.25/1.65、PIN 2.48/1.543 和 −1.07%。"
)
...
gaps = [
    ...
    f"(child 1634353344 also ABSENT; 17:00 CST 8/20 / 09:00Z)；{gap_parents}。"
    + (
        f"TT vs LGD 已挂父盘 {pin_tt_parent_id} period-0 ML {pin_tt}/{pin_lgd} Am {pin_tt_am}/{pin_lgd_am}。"
        if pin_tt_lgd_listed and pin_tt is not None
        else "无 TT vs LGD（8/23 仍未挂上）。"
    )
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 11:34 prematch quotes; JDG/TT not reused (ended)"
        if dagger
...
payload = {
    ...
    "tt_lgd_days_to_start": round(max(days_to_tt, 0.0), 2),
    ...
    "sources": {
        ...
        "PIN": (
            ...
            + (
                f"TT/LGD parent {pin_tt_parent_id} listed period-0 ML {pin_tt}/{pin_lgd} Am {pin_tt_am}/{pin_lgd_am} status={pin_tt_ml_status}; "
                if pin_tt_lgd_listed and pin_tt is not None
                else "no TT/LGD parent; "
            )
            + "today's parents: "
            + (
                "; ".join(
        ...
        "Stake": (
            ...
            + ("(429 †, reused 11:34); " if dagger else "(fresh, no 429); ")
    ...
    "vs_1134": {
        ...
        "TT vs LGD": {
            ...
            "stake": book_pair(base_stake_tt, base_stake_lgd, stake_tt2, stake_lgd) + ("（429†沿用11:34）" if dagger else ""),
            "duel": book_pair(base_duel_tt, base_duel_lgd, duel_tt2, duel_lgd) + ("（429†沿用11:34）" if dagger else ""),
            "pin": book_pair(base_pin_tt, base_pin_lgd, pin_tt, pin_lgd),
            ...
            "path_pin_tt": f"{rate_pct_str(base_rates['pin_tt'])}→{rate_pct_str(cur_rates.get('pin_tt', base_rates['pin_tt']))}",
            "path_pin_lgd": f"{rate_pct_str(base_rates['pin_lgd'])}→{rate_pct_str(cur_rates.get('pin_lgd', base_rates['pin_lgd']))}",
            "status": "非滚球→非滚球",
        },
    ...
    "table1": [
        ...
        },
        {
            ...
            "pin_american": {"home": pin_tt_am, "away": pin_lgd_am} if pin_tt_am is not None else None,
            "pin_limit": pin_tt_limit,
            "pin_status": pin_tt_ml_status or pin_tt_parent_status,
            "pin_live": False,
        },
    ],
    "table2": table2_json,
