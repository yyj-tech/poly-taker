#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-23 ~09:31 CST vs 22:33. Analysis only."""
...
# 429: reuse 22:33 TT vs LGD last-success quotes with †. JDG ended → keep —.
STALE_2233_LGD = {
    "stake_a": 2.1,
    "stake_b": 1.75,
    "duel_a": 2.16,
    "duel_b": 1.74,
    "stake_updated": "2026-08-22T13:57:20.483Z",
    "duel_updated": "2026-08-22T13:30:18.451Z",
}
...
if s_b_lgd and pm_b_lgd is not None and pin_tt:
    paths.append(
        (
            "PIN",
            path(s_b_lgd, pm_b_lgd, pin_tt, "PIN", "TT", "LGD"),
            bool(s_b_lgd and s_b_lgd > 2000),
        )
    )
if s_a_tt and pm_a_tt is not None and pin_lgd:
    paths.append(
        (
            "PIN",
            path(s_a_tt, pm_a_tt, pin_lgd, "PIN", "LGD", "TT"),
            bool(s_a_tt and s_a_tt > 2000),
        )
    )
...
# baseline 22:33 CST values
base_pm_tt, base_pm_lgd = 0.46, 0.55
base_stake_tt, base_stake_lgd = 2.1, 1.75
base_duel_tt, base_duel_lgd = 2.16, 1.74
base_sz_tt, base_sz_lgd = 5079.98, 2953.09
base_rates = {
    "duel_tt": -0.01938597,  # Duel buy TT
    "stake_tt": -0.03194272,
    "duel_lgd": -0.03885509,
    "stake_lgd": -0.03581166,
}
base_best = -0.01938597
...
if dagger:
    books_move_detail = "Stake/Duel 429 † reused 22:33, not treated as live move"
elif books_moved:
    books_move_detail = (
...
for book, r, thick in paths_sorted:
    if book == "PIN":
        ...
        note = f"LPL PIN现价，{lim}"
    elif book == "Duel":
        note = "LPL Duel 429†沿用22:33，限额未知" if dagger else "LPL Duel现价，限额未知"
    else:
        note = "LPL Stake 429†沿用22:33，限额未知" if dagger else "LPL Stake现价，限额未知"
    thick_note = "；档1异常厚" if thick else ""
    hv_note = "可升格" if r["hv"] else "不升格"
    event = f"TT vs LGD（{note}；本单按PM档1；{hv_note}{thick_note}）"
...
if pin_tt_lgd_listed and pin_tt is not None and pin_lgd is not None:
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，TT vs LGD 沿用 22:33 赛前价；JDG vs TT 已结束不沿用陈价。"
        if dagger
...
md.append(
    ...
    + ("（429†陈价）" if dagger else "")
    + "；"
    + f"PIN {book_pair(None, None, pin_tt, pin_lgd)}。最佳利率 {rate_pct_str(base_best)}（Duel买TT）→{rate_pct_str(best_rate)}（{best['book']}买{best['book_team']}，{pp_txt(best_pp)}个百分点）。"
...
reason_txt = "；".join(reasons) if reasons else (
    ...
    "PM ask<0.02、Stake/Duel 无实质移动、PIN未新挂、未转滚球、档深变化不触发；上一轮已报 Duel 2.16/1.74、Stake 2.1/1.75 和 −1.94%。"
)
...
gaps = [
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 22:33 prematch quotes; JDG/TT not reused (ended)"
        if dagger
...
payload = {
    ...
    "sources": {
        ...
        "Stake": (
            ...
            + ("(429 †, reused 22:33); " if dagger else "(fresh, no 429); ")
    ...
    "vs_2233": {
        ...
        "TT vs LGD": {
            ...
            "pin": book_pair(None, None, pin_tt, pin_lgd),
