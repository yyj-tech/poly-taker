#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-22 ~17:31 CST vs 16:34. Analysis only."""
...
# 429: reuse 16:34 TT vs LGD last-success quotes with †. JDG ended → keep —.
STALE_1634_LGD = {
    "stake_a": 2.0,
    "stake_b": 1.82,
    "duel_a": 1.99,
    "duel_b": 1.88,
    "stake_updated": "2026-08-22T05:41:32.433Z",
...
# baseline 16:34 CST values
base_pm_tt, base_pm_lgd = 0.46, 0.55
base_stake_tt, base_stake_lgd = 2.0, 1.82
base_duel_tt, base_duel_lgd = 1.99, 1.88
base_sz_tt, base_sz_lgd = 566.95, 1204.5
base_rates = {
    "duel_tt": -0.05599713,  # Duel buy TT
    "stake_tt": -0.05375276,
    "duel_lgd": 0.00237755,
    "stake_lgd": -0.01493724,
}
...
if dagger:
    books_move_detail = "Stake/Duel 429 † reused 16:34, not treated as live move"
elif books_moved:
    books_move_detail = (
...
reason_txt = "；".join(reasons) if reasons else (
    ...
    "PM ask<0.02、Stake/Duel 无实质移动、未转滚球；上一轮已报 Stake 2.00/1.82 和 +0.24%。"
)
...
payload = {
    ...
    "vs_1634": {
        ...
        "lgd_l1_size_delta": round((s_b_lgd or 0) - 1204.5, 2),
        "tt_l1_size_delta": round((s_a_tt or 0) - 566.95, 2),
    },
    "table1": [
        {
            "状态": "结束",
