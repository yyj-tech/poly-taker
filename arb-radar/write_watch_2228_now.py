#!/usr/bin/env python3
"""Write watch-jdg-tt.md/json for 2026-08-22 ~22:28 CST vs 21:38. Analysis only."""
...
# 429: reuse 21:38 TT vs LGD last-success quotes with †. JDG ended → keep —.
STALE_2138_LGD = {
    "stake_a": 2.15,
    "stake_b": 1.7,
    "duel_a": 2.16,
    "duel_b": 1.74,
    "stake_updated": "2026-08-22T09:34:31.372Z",
...
if dagger and lgd_books_missing:
    stake_tt2 = STALE_2138_LGD["stake_a"]
    stake_lgd = STALE_2138_LGD["stake_b"]
    duel_tt2 = STALE_2138_LGD["duel_a"]
    duel_lgd = STALE_2138_LGD["duel_b"]
...
if dagger:
    ...
    oai_live_lgd = False
    if not stake_upd:
        stake_upd = STALE_2138_LGD["stake_updated"]
    if not duel_upd:
        duel_upd = STALE_2138_LGD["duel_updated"]
...
# baseline 21:38 CST values
base_pm_tt, base_pm_lgd = 0.46, 0.55
base_stake_tt, base_stake_lgd = 2.15, 1.7
base_duel_tt, base_duel_lgd = 2.16, 1.74
base_sz_tt, base_sz_lgd = 4856.45, 16704.16
base_rates = {
    "duel_tt": -0.01938597,  # Duel buy TT
    "stake_tt": -0.02145224,
    "duel_lgd": -0.03885509,
    "stake_lgd": -0.05118703,
}
...
if dagger:
    books_move_detail = "Stake/Duel 429 † reused 21:38, not treated as live move"
elif books_moved:
    books_move_detail = (
...
for book, r, thick in paths_sorted:
    ...
    if dagger:
        note = ("LPL Duel 429†沿用21:38" if book == "Duel" else "LPL Stake 429†沿用21:38")
...
md.append(
    ...
    + (
        "Stake/Duel 经 odds-api.io LPL Split 3 本轮 429 †，TT vs LGD 沿用 21:38 赛前价；JDG vs TT 已结束不沿用陈价。"
        if dagger
...
md.append(
    ...
    + ("（429†陈价）" if dagger else "")
    + "；"
    + f"PIN 仍无。最佳利率 {rate_pct_str(base_best)}（Duel买TT）→{rate_pct_str(best_rate)}（{best['book']}买{best['book_team']}，{pp_txt(best_pp)}个百分点）。"
...
reason_txt = "；".join(reasons) if reasons else (
    ...
    "PM ask<0.02、Stake/Duel 无实质移动、未转滚球、档深变化不触发；上一轮已报 Duel 2.16/1.74、Stake 2.15/1.70 和 −1.94%。"
)
...
gaps = [
    ...
    (
        "odds-api.io 429 † this round; Stake/Duel TT vs LGD reused 21:38 prematch quotes; JDG/TT not reused (ended)"
        if dagger
...
payload = {
    ...
    "sources": {
        ...
        "Stake": (
            ...
            + ("(429 †, reused 21:38); " if dagger else "(fresh, no 429); ")
    ...
    "vs_2138": {
        ...
        "lgd_l1_size_delta": round((s_b_lgd or 0) - 16704.16, 2),
        "tt_l1_size_delta": round((s_a_tt or 0) - 4856.45, 2),
    },
    "table1": [
        {
            "状态": "结束",
